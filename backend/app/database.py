        ...
        try:
            for provider, model, price_in, price_out in default_pricing:
                await conn.execute(text("""
                    ...
                    SELECT gen_random_uuid(), CAST(:provider AS VARCHAR(50)), CAST(:model AS VARCHAR(100)), CAST(:price_in AS FLOAT), CAST(:price_out AS FLOAT), 'EUR'
                    WHERE NOT EXISTS (SELECT 1 FROM ai_model_pricing WHERE provider = CAST(:provider AS VARCHAR(50)) AND model_name = CAST(:model AS VARCHAR(100)))
