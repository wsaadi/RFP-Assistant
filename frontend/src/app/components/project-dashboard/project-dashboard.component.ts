...
export class ProjectDashboardComponent implements OnInit, OnDestroy {
  ...
  addNewChapter(generateAfter = false): void {
    ...
    // Determine the response_document_id and order
    let responseDocId = this.newChapterDocId;
    if (!responseDocId && this.redactionDocs.length === 1) {
      responseDocId = this.redactionDocs[0].id;
    }

    // Find max order among existing chapters for this document
    let maxOrder = 0;
    for (const group of this.groupedChapters) {
      for (const ch of group.chapters) {
        if (ch.order > maxOrder) maxOrder = ch.order;
    ...
    if (responseDocId) {
      // We need to set parent via the backend - the create endpoint uses parent_id for sub-chapters
      // For top-level chapters with a document association, we need to update after creation
    }
    ...
    this.api.createChapter(this.projectId, data).subscribe({
      next: (newChapter) => {
        ...
        this.loadAll();
        if (generateAfter) {
          // Wait for loadAll to finish, then generate
          setTimeout(() => this.aiGenerate(newChapter.id), 1500);
        }
      },
      error: (err) => {
